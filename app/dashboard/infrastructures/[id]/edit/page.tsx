...
export default function EditInfrastructurePage() {
  ...
  const [existingPhotos, setExistingPhotos] = useState<Array<{ url: string; uploadedAt?: string }>>([])
  ...
  const fetchInfrastructure = async () => {
    try {
      ...
      setFormData({
        ...
        etat: infra.etat || 'bon',
        niveauFrequentation: infra.niveau_frequentation || infra.niveauFrequentation || 'moyen',
        pmr: infra.accessibilite?.pmr || false,
        enfants: infra.accessibilite?.enfants || false,
        equipements: infra.equipements || [],
        horaires: infra.horaires || formData.horaires
      })

      if (infra.photos && infra.photos.length > 0) {
        setExistingPhotos(infra.photos)
      }
    } catch (err: any) {
  ...
  const handleHorairesChange = (jour: string, field: string, value: string | boolean) => {
    setFormData(prev => ({
      ...prev,
      horaires: {
        ...prev.horaires,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (!formData.latitude || !formData.longitude) {
        throw new Error('Les coordonnées GPS (latitude et longitude) sont requises')
      }
      ...
      setError(err.response?.data?.message || err.message || 'Erreur lors de la mise à jour de l\'infrastructure')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Link
            href={`/dashboard/infrastructures/${id}`}
            className="text-gray-600 hover:text-gray-900"
          >
            <FiArrowLeft className="w-6 h-6" />
          </Link>
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Modifier l'Infrastructure</h1>
            <p className="text-gray-600 mt-1">Mettre à jour les informations de l'infrastructure</p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow p-6 space-y-6">
        ...
        {/* Localisation */}
        <div className="space-y-4">
          <h2 className="text-xl font-semibold text-gray-900 border-b pb-2">Localisation</h2>
        ...
        {/* Contact */}
        <div className="space-y-4">
          <h2 className="text-xl font-semibold text-gray-900 border-b pb-2">Contact</h2>
        ...
        <div className="space-y-4">
          <h2 className="text-xl font-semibold text-gray-900 border-b pb-2">État et accessibilité</h2>
        ...
        {/* Photos */}
        <div className="space-y-4">
          <h2 className="text-xl font-semibold text-gray-900 border-b pb-2">Photos (optionnel)</h2>
          
          {/* Photos existantes */}
          {existingPhotos.length > 0 && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Photos existantes</label>
        ...
        {/* Équipements */}
        <div className="space-y-4">
          <h2 className="text-xl font-semibold text-gray-900 border-b pb-2">Équipements</h2>
        ...
        {/* Horaires */}
        <div className="space-y-4">
          <h2 className="text-xl font-semibold text-gray-900 border-b pb-2">Horaires d'ouverture</h2>
            ...
            {jours.map((jour) => (
              <div key={jour} className="flex items-center gap-4 p-3 bg-gray-50 rounded-lg">
                <label className="flex items-center gap-2 min-w-[120px]">
                  <input
                    type="checkbox"
                    checked={formData.horaires[jour as keyof typeof formData.horaires].ouvert}
                    onChange={(e) => handleHorairesChange(jour, 'ouvert', e.target.checked)}
                    className="w-4 h-4 text-primary-600 border-gray-300 rounded focus:ring-primary-500"
                  />
                  <span className="text-sm font-medium text-gray-700">{joursLabels[jour]}</span>
                </label>
                
                {formData.horaires[jour as keyof typeof formData.horaires].ouvert && (
                  <div className="flex items-center gap-2 flex-1">
                    <input
                      type="time"
                      value={formData.horaires[jour as keyof typeof formData.horaires].debut}
                      onChange={(e) => handleHorairesChange(jour, 'debut', e.target.value)}
                      className="px-3 py-1 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                    />
                    <span className="text-gray-500">-</span>
                    <input
                      type="time"
                      value={formData.horaires[jour as keyof typeof formData.horaires].fin}
                      onChange={(e) => handleHorairesChange(jour, 'fin', e.target.value)}
                      className="px-3 py-1 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                    />
                  </div>
                )}
